...
export default async function AdminPage() {
  ...
  const [concertsResult, linkCardsResult, subscribersResult] =
    await Promise.all([
    ...
    <div>
      <Tabs defaultValue='concerts'>
        <TabsList className='mb-8 bg-transparent p-0 h-auto gap-0'>
          ...
          <TabsTrigger
            value='link-cards'
            className='rounded-none border border-r-0 px-5 py-2.5 text-[14px] data-[state=active]:bg-[rgba(209,176,249,0.08)] data-[state=active]:border-[rgba(209,176,249,0.18)] data-[state=active]:text-lavender data-[state=active]:font-semibold data-[state=inactive]:border-[rgba(255,255,255,0.06)] data-[state=inactive]:bg-transparent data-[state=inactive]:text-muted-foreground data-[state=active]:shadow-none'
          >
          ...
          >
            Newsletter ({subscribersResult.data?.length ?? 0})
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value='newsletter' className='mt-0'>
          <NewsletterAdmin subscribers={subscribersResult.data ?? []} />
        </TabsContent>
      </Tabs>
    </div>
  );
